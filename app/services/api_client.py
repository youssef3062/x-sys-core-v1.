...
def handle_api_response(response, success_message=None):
    ...
    try:
        ...
        logger.error(
            f"API response parsing failed: {e}. Status: {response.status_code}, "
            f"URL: {getattr(response, 'url', 'unknown')}, Content-Type: {response.headers.get('Content-Type')}"
        )
...
def safe_get(path: str, params: dict = None, timeout: int = 10):
    if not requests:
        logger.error("requests dependency missing; cannot perform outbound HTTP")
        return None
    try:
        url = api_url(path)
